# Easy Japanese: Anime & J-Pop Vocabulary Pack

> Learn Japanese the way fans actually consume it. Anime, J-pop, drama phrasebook for JLPT N5/N4 learners worldwide.

3 free sample lessons + a free Anki sample deck + 50+ premium lessons + Anki deck for $29.

## Why This Pack

Most Japanese textbooks teach office formalities. But fans of anime, J-pop, and J-drama want to understand:
- Why does this character say it that way?
- What's the difference between "ohayou" and "ohayou gozaimasu"?
- What does "anpi fumei" mean in this song?

This pack breaks down real expressions from cultural contexts (without redistributing copyrighted source material), with grammar component breakdowns, JLPT level tags, and Anki-ready cards.

## Free Samples (Public)

Read these without paying anything:

**Anki sample deck:**

- [Download the free sample deck](samples/easy-japanese-sample.apkg) - 20 notes / 40 cards from the JLPT N5/N4 vocabulary pack

**Basics (perfect if you're starting out):**

1. [**"Good Morning" - Ohayou vs Ohayou Gozaimasu**](samples/01-greetings-ohayou.md) - Casual vs formal greetings, component breakdown
2. [**Hiragana Pronunciation Reading**](samples/02-hiragana-pronunciation.md) - How to actually pronounce the kana you keep seeing
3. [**"Anpi Fumei" - Whereabouts Unknown**](samples/03-anpiufumei-meaning.md) - Vocabulary you hear in news and dramas

**J-Pop deep dives (real songs, copyright-safe educational analysis):**

4. [**back number - "Happy End"**](samples/04-jpop-back-number-happy-end.md) - Vocabulary of regret, ～のに particle, hazu ga nai grammar
5. [**back number - "Hanataba"**](samples/05-jpop-back-number-hanabata.md) - Romantic emotional vocabulary
6. [**Official Hige Dandism - "Stardust"**](samples/06-jpop-higedan-stardust.md) - Modern pop grammar patterns

If these feel useful, the paid pack contains 50+ more lessons in the same depth + an Anki deck with 98 cards.

## What's In The Paid Pack ($29 USD)

| Item | Count |
|---|---|
| Lesson Markdown files (grammar breakdown + cultural context) | 50+ |
| Anki deck (.apkg ready to import) | 1 (~300 cards) |
| JLPT N5/N4 vocabulary CSV | 1 |
| PDF compilation (printable) | 1 |
| Romaji + IPA pronunciation guide | included in each lesson |
| Translation in EN + ES + VI (some lessons) | yes |

### Lesson Topics Covered

- Greetings & social markers (casual ↔ formal)
- Hiragana / Katakana mnemonic atlas
- JLPT N5 kanji memory atlas
- JLPT N4 exam-derived kanji
- J-pop common expressions (from popular songs - lyric-safe metadata only)
- Anime / drama phrasebook situations
- Particle usage in real speech (は, が, を, に, で)
- Politeness levels (です/ます vs casual)
- Time expressions
- Numbers, counters, money
- Family, food, travel vocabulary
- Common verbs and conjugations

## Rights / Copyright Safety

This pack does NOT redistribute:
- Anime subtitles or scripts
- Song lyrics (full text)
- Manga panels
- Audio files

The pack DOES include:
- Original grammar explanations
- Vocabulary breakdowns
- Cultural context written by the author
- References (artist name + song title for context, no lyrics)
- Anki cards with author-written example sentences

All content is original commentary or fair-use educational reference. Buyer assumes responsibility for personal use only (no resale or redistribution).

## How To Buy

### Option 1 - PayPal (International) - $29 USD
1. Pay $29 USD via PayPal.Me: https://www.paypal.me/sks7178/29
2. Open a [GitHub Issue with Buy template](https://github.com/duct-tape2/japanese-anki-pack/issues/new) - include your PayPal transaction ID
3. Within 5 min (during JST/KST hours) you receive a download link with the full ZIP

### Option 2 - Wise / Crypto
- Wise: USD or JPY accepted, email `sks7178@gmail.com` for handle
- BTC / ETH: contact via GitHub Issue for current address

### Option 3 - Korean Buyers (₩39,000)
- Toss / 계좌이체 / 카카오페이 환영. GitHub Issue 열고 region: Korea 표시.

## Refund Policy

Full refund within 14 days if:
- ZIP files don't unzip
- Anki deck doesn't import to Anki 24.x
- Major factual error in vocabulary cards

No refunds for "I already knew this" - the free samples exist so you can check the depth/style before buying.

## Who This Is For

- JLPT N5-N4 learners stuck on textbook formality
- Anime / J-pop fans who want to actually understand lyrics + dialogue
- Self-learners using Anki for spaced repetition
- Travelers preparing for Japan trip

## Who This Is NOT For

- Advanced learners (N3+) - this is N5-N4 focus
- People wanting full song lyrics translated (we don't redistribute copyrighted text)
- People wanting audio recordings (text-only pack)

## Contact

- GitHub: [@duct-tape2](https://github.com/duct-tape2)
- Email: `sks7178@gmail.com`
- Issue template: open a new issue on this repo

---

Built by [@duct-tape2](https://github.com/duct-tape2) using a multi-lingual content generation pipeline. Original Korean-source content rewritten in English with native-speaker review.
